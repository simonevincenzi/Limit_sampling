...
uppidri_growth_list = list()

rm(uppidri_growth_nls_df)

uppidri_growth_nls_df = tibble(linf = rep(0,length(max_year_v)),
...
uppidri_growth_list[[j_n]] = synth.list.3$coh.par %>%
...
uppidri_growth_nls_df$linf[j_n] = summary(nls_fit)$coefficients[which(rownames(summary(nls_fit)$coefficients) == "linf"),1]
uppidri_growth_nls_df$k[j_n] = summary(nls_fit)$coefficients[which(rownames(summary(nls_fit)$coefficients) == "k"),1]
uppidri_growth_nls_df$t0[j_n] = summary(nls_fit)$coefficients[which(rownames(summary(nls_fit)$coefficients) == "t0"),1]

uppidri_growth_nls_df$linf_lcl[j_n] = nls_confint[which(rownames(nls_confint) == "linf"),1]
uppidri_growth_nls_df$linf_ucl[j_n] = nls_confint[which(rownames(nls_confint) == "linf"),2]
uppidri_growth_nls_df$k_lcl[j_n] = nls_confint[which(rownames(nls_confint) == "k"),1]
uppidri_growth_nls_df$k_ucl[j_n] = nls_confint[which(rownames(nls_confint) == "k"),2]
uppidri_growth_nls_df$t0_lcl[j_n] = nls_confint[which(rownames(nls_confint) == "t0"),1]
uppidri_growth_nls_df$t0_ucl[j_n] = nls_confint[which(rownames(nls_confint) == "t0"),2]
uppidri_growth_nls_df$Samples[j_n] = length(unique(data_growth$Mark))
uppidri_growth_nls_df$n_data[j_n] = nrow(data_growth)
uppidri_growth_nls_df$expL1[j_n] = predict(nls_fit, newdata = data.frame("Age" = 1))
uppidri_growth_nls_df$expL2[j_n] = predict(nls_fit, newdata = data.frame("Age" = 2))
uppidri_growth_nls_df$expL3[j_n] = predict(nls_fit, newdata = data.frame("Age" = 3))
uppidri_growth_nls_df$final_year[j_n] = max_year_v[j_n]
uppidri_growth_nls_df$tot_year[j_n] = 1 + (max_year_v[j_n] - min(data_growth$Year))
}
