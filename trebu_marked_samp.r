...
data_df_complete = filter(trebu_df,  !is.na(Mark_cor) & Age_cor>0 & !is.na(Month) &
                Year >=2006)
...
max_year_v = c(2009,2010,2011,2012,2013,2014)
mod_list = list()
annual = 1
...
design.Phi=list(static=c("Coh_n"))
design.p=list(static=c("Coh_n"))
...
mod_list[[j_n]] = list("mod_cost" = predict(test[[2]], ddl = test.ddl, se = T)$Phi %>%  add_column(samples = nrow(data.marked), final_year = maxyear, tot_year = 1 + (final_year - minyear)),"table" = test$model.table)
}
